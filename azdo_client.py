...
class AzureDevOpsClient:
    ...
    def get_pr_added_lines_only(self, repo, pr_id):
        ...
        # Tentar obter diff por branches
        diff_data = self.get_pr_diff(repo, pr_id)

        # Se falhar (branch apagada), tentar obter por commits
        if not diff_data:
            print("⚠️  Branch source foi apagada, tentando obter alterações pelos commits...")
            return self._get_pr_changes_from_commits(repo, pr_id, pr_data)
    ...
    def _get_pr_changes_from_commits(self, repo, pr_id, pr_data):
        """Método alternativo para obter alterações quando a branch foi apagada"""
        try:
            # Obter commits do PR
            commits_data = self.get_pr_commits(repo, pr_id)
            if not commits_data or not commits_data.get('value'):
                print("✗ Não foi possível obter commits do PR")
                return None

            commits = commits_data.get('value', [])
            last_commit_id = pr_data.get('lastMergeSourceCommit', {}).get('commitId')
            base_commit_id = pr_data.get('lastMergeTargetCommit', {}).get('commitId')

            if not last_commit_id or not base_commit_id:
                print("✗ IDs de commits não encontrados")
                return None

            # Buscar diff entre commits
            project_encoded = urllib.parse.quote(self.project)
            url = f"{self.base_url}/{project_encoded}/_apis/git/repositories/{repo}/diffs/commits?baseVersionType=commit&baseVersion={base_commit_id}&targetVersionType=commit&targetVersion={last_commit_id}&api-version=7.0"

            response = requests.get(url, headers=self.headers, auth=self.auth)

            if response.status_code != 200:
                print(f"✗ Erro ao buscar diff por commits: {response.status_code}")
                return None
            ...
            print(f"✓ Obtidas alterações usando commits (método alternativo)")
            return {
                'files': files_with_additions,
                'total': len(files_with_additions),
                'source_branch': 'N/A (branch apagada)',
                'target_branch': pr_data.get('targetRefName', '').replace('refs/heads/', ''),
                'method': 'commits'
            }
