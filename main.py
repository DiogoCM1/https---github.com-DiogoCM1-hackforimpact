...
REPO = "MedicineOneLibrary"  # Nome do repositório Azure DevOps
PR_ID = 14052  # ID do Pull Request

# Dados opcionais da issue (deixar None se não houver issue associada)
ISSUE_KEY = None#"M1API-1060" #"M1-28446"  # Key da issue do Jira (ou None)

# O que gerar (True/False)
GENERATE_DOCUMENTATION = False  # Gerar documentação técnica
...
class WorkflowOrchestrator:
    ...
    def __init__(self, repo, pr_id, issue_key=None,
        ...
        Args:
            ...
            issue_key (str, optional): Key da issue do Jira (ex: M1-28446). Pode ser None.
...
if __name__ == "__main__":
    try:
        ...
        print("Mensagem de debug", flush=True)
        logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
        logging.info("Buscando PR %s", PR_ID)
 

        print("AZDO_ORG =", os.getenv("AZDO_ORG"))
        print("AZDO_PAT =", os.getenv("AZDO_PAT"))
        print("AZDO_REPO =", os.getenv("AZDO_REPO"))
        orchestrator.run()
